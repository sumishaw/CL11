...
class _HomePageState extends State<HomePage> with WidgetsBindingObserver {
  ...
  bool    subtitleHidden   = false; // Caption Lens's own overlay, background/hide mode
  ...
  Future<void> _setSubtitleHidden(bool hidden) async {
    await _ch.invokeMethod('setHindiSubtitleHidden', {'hidden': hidden});
    if (mounted) setState(() => subtitleHidden = hidden);
  }

  Future<void> _openLiveCaptionSettings() async {
    await _ch.invokeMethod('openLiveCaptionSettings');
  }
  String  statusMsg        = '';
  ...
  Widget _buildBackgroundModeControls() {
    const accent = Color(0xFFFF3B3B);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Watch Without Captions Covering Screen',
            style: TextStyle(color: Colors.white54, fontSize: 12,
                fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),

        // ── Caption Lens's own Hindi subtitle — REAL, fully working toggle ──
        // Hiding this does NOT stop translation or Hindi speech — both keep
        // running exactly as before, only the on-screen text is hidden.
        Row(children: [
          const Expanded(
            child: Text('Hide Caption Lens Hindi subtitle',
                style: TextStyle(color: Colors.white70, fontSize: 13)),
          ),
          GestureDetector(
            onTap: () => _setSubtitleHidden(!subtitleHidden),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 200),
              width: 48, height: 26,
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(13),
                color: subtitleHidden ? accent : Colors.white12,
              ),
              child: AnimatedAlign(
                duration: const Duration(milliseconds: 200),
                alignment: subtitleHidden
                    ? Alignment.centerRight : Alignment.centerLeft,
                child: Container(
                  margin: const EdgeInsets.all(3),
                  width: 20, height: 20,
                  decoration: BoxDecoration(
                    shape: BoxShape.circle,
                    color: subtitleHidden ? Colors.white : Colors.white38,
                  ),
                ),
              ),
            ),
        ...
        Container(
          ...
          child: Text(
            subtitleHidden
                ? '🔇 Subtitle hidden — Hindi speech and translation keep running normally'
                : 'Hindi subtitle text is shown on screen as usual',
            style: const TextStyle(color: Colors.white38, fontSize: 11),
          ),
        ),
        ...
        // ── Android's own Live Caption — NOT a real toggle ──────────────────
        // We deliberately do NOT show a fake switch here. There is no public
        // API for this app to hide another app's system overlay window, so a
        // toggle that looked like it worked would be misleading. This is a
        // clearly-labeled shortcut to system settings instead — the user has
        // to do this part manually, on purpose.
        const Text('Live Caption (Android system feature)',
            style: TextStyle(color: Colors.white70, fontSize: 13)),
        const SizedBox(height: 6),
        Container(
          padding: const EdgeInsets.all(8),
          ...
          child: const Text(
            'Caption Lens can\'t hide Live Caption\'s own bubble — that '
            'belongs to Android, not this app. Tap it once to minimize it '
            'to a small pill, or turn it off in system settings below. '
            'Caption Lens keeps reading it either way, as long as it\'s not '
            'fully turned off.',
            style: TextStyle(color: Colors.white38, fontSize: 11),
          ),
        ),
        const SizedBox(height: 8),
        GestureDetector(
          onTap: _openLiveCaptionSettings,
          child: Container(
            width: double.infinity,
            ...
            child: const Text('Open Live Caption Settings',
                textAlign: TextAlign.center,
                style: TextStyle(color: Colors.white70, fontSize: 13,
                    fontWeight: FontWeight.bold)),
          ),
